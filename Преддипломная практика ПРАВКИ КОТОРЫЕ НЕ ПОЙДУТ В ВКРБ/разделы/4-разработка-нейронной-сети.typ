...
#рис(image("../материалы/общая-архитектура.jpg", width: 100%))[ Архитектура нейронной сети ] <общая-архитектура>
...
В рамках ВКРБ рассматриваются только три дефекта, и поэтому в обучении не будут участвовать все 840000 изображений, но на их базе можно сформировать достаточную выборку изображений с нужными дефектами. Из всего спектра искажений KADIS-700K используются только следующие типы:
...
Модифицированный код представлен в листинге @матлаб-модификация.
...
В результате сгенерированы искажённые изображения, соответсвующие выбранным дефектам ВКРБ и сохраненные в формате, близком к условиям практического использования в приложении.
...
Скрипт, разделяющий датасет на обучающую и валидационную выборки содержится в листинге @скрипт-разметка.
...
В результате такой процедуры обеспечивается отсутствие пересечения сцен между выборками: одна и та же исходная фотография и её искажённая версия могут принадлежать только обучающей или только валидационной части.
...
#рис(image("../материалы/второе_поколение_accuracy_доп.jpg", width: 60%))[ Точность предсказаний второй итерации ] <второе-точность>
...
#рис(image("../материалы/общая-архитектура-2.jpg", width: 80%))[ Архитектура нейронной сети ] <общая-архитектура-2>
