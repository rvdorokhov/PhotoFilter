...
#рис(image("../материалы/второе_поколение_accuracy_доп.jpg", width: 80%))[ Точность предсказаний второй итерации ] <второе-точность>
...
После экспериментов с первой версией модели была проведена доработка архитектуры. Базовая схема "один общая часть + несколько выходов" была сохранена. Основное изменение связано с добавлением четвертой головы nigth, предназначенной для распознавания ночных сцен. Это связано с тем, что under может ошибочно срабатывать на ночных кадрах, где низкая яркость является нормой, а не дефектом. Голова night реализована как отдельный выход модели и используется в прикладной логике: при высокой вероятности ночной сцены фильтрацию по under можно ослаблять или отключать, чтобы не браковать корректные ночные фотографии - это должно помочь повысить точность головы under и снизить ее количество ложноположительных срабатываний непосредственно в прикладном приложении. Примечание - здесь и далее понятие "ночная сцена" не обязательно означает  прямом смысле ночную сцену (хотя и в том числе), но так же включает в себя темные и слабоосвещенные фотографии, сделанные, например, в условиях плохого освещения. Итоговая архитектура схематично представлена на рисунке @общая-архитектура-2.
...
В результате для хранения разметки был сформирован единый файл night_labels.csv с дополнительным полем nigth, которого нет в первом датасете.
...
Скрипт, разделяющий датасет на обучающую и валидационную выборки содержится в @скрипт-разметка-2 и в приложении A.
...
Качество голов blur/under/over оценивалось на обычной валидации val.csv (8954 изображения) при порогах 0.5. Итоговые значения не отличаются от значений второй итерации - что не удивительно, потому что изменения в обучение трех изначальных голов не вводились.

Качество головы night оценивалось отдельно на night_val.csv (4283 изображения) при пороге 0.5. Получены значения: TP=1414, FP=28, FN=59, TN=2782, что соответствует precision 0.9806, recall 0.9599, F1 0.9702 и accuracy 0.9797. Эти результаты показывают, что модель уверенно отделяет ночные сцены от дневных, а значит может использоваться как надёжный "контекстный фильтр" для under.
